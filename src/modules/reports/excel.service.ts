  ...
  async importProducts(fileBuffer: Buffer): Promise<ProductImportResult> {
    ...
    headerRow.eachCell((cell, colNumber) => {
      ...
      if (val.includes("code")) colMap.set("productCode", colNumber);
        ...
        colMap.set("lowStockLevel", colNumber);
    ...
    await this.prisma.$transaction(
      async (tx) => {
        ...
        for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
          ...
          const barcode =
            getCellValue(row, "barcode")?.toString().trim() || null;
          const categoryName = getCellValue(row, "category")?.toString().trim();
          const brandName = getCellValue(row, "brand")?.toString().trim();
          const unitName =
            getCellValue(row, "unit")?.toString().trim() || "PCS";
          const hsnCode =
            getCellValue(row, "hsnCode")?.toString().trim() || null;
          const taxRateVal = Number(getCellValue(row, "taxRate") || 0);
          const taxTypeStr = getCellValue(row, "taxType")
            ?.toString()
            .trim()
            .toUpperCase();
          ...
          const description =
            getCellValue(row, "description")?.toString().trim() || null;
          const activeVal = getCellValue(row, "isActive")
            ?.toString()
            .trim()
            .toUpperCase();
          const isActive =
            activeVal === "NO" || activeVal === "FALSE" ? false : true;
