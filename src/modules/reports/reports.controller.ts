...
export class ReportsController {
  ...
  @ApiOperation({ summary: 'Export products catalogue with stock and prices to Excel' })
  ...
  async importProducts(@UploadedFile() file: Express.Multer.File) {
    if (!file || !file.buffer) {
      throw new BadRequestException('Excel file is required in multipart form data under field "file"');
    }
  ...
  @ApiOperation({ summary: 'Export any tabular report directly into an Excel spreadsheet' })
  async exportReportToExcel(
    ...
    switch (reportType.toLowerCase()) {
      ...
      case 'purchases': {
        ...
        const result = await this.reportsService.getPurchaseReport(exportFilters);
        rows = result.data;
        break;
      }
      case 'profit': {
      ...
      case 'customer-outstanding': {
        ...
        const result = await this.reportsService.getCustomerOutstanding(exportFilters);
        rows = result.data;
        break;
      }
      case 'supplier-outstanding': {
        ...
        columns = [
          ...
          { header: 'Total Paid', key: 'totalPaid', width: 16 },
          { header: 'Outstanding Payable', key: 'outstandingBalance', width: 18 },
        ];
        const result = await this.reportsService.getSupplierOutstanding(exportFilters);
        rows = result.data;
        break;
      }
      case 'payments': {
        ...
        const result = await this.reportsService.getPaymentReport(exportFilters);
        rows = result.data;
        break;
      }
      case 'expenses': {
        ...
        const result = await this.reportsService.getExpenseReport(exportFilters);
        rows = result.data;
        break;
      }
      case 'gst-summary': {
      ...
      case 'daily-sales': {
        ...
        const result = await this.reportsService.getDailySalesReport(exportFilters);
        rows = result.data;
        break;
      }
      case 'monthly-sales': {
        ...
        const result = await this.reportsService.getMonthlySalesReport(exportFilters);
        rows = result.data;
        break;
      }
      case 'top-products': {
        ...
        const result = await this.reportsService.getTopProductsReport(exportFilters);
        rows = result.data;
        break;
      }
      case 'low-stock': {
        ...
        const result = await this.reportsService.getLowStockReport(exportFilters);
        rows = result.data;
        break;
      }
      default:
    ...
    const buffer = await this.excelService.exportReportToExcel(title, columns, rows);
